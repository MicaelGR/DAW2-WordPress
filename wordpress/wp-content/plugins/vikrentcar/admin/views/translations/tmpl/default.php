					...
					if (count($json_def_values) > 0) {
						...
						foreach ($json_def_values as $jkey => $jval) {
							...
							<div class="vrc-translations-element-row vrc-translations-element-row-nested" data-reference="<?php echo $ltag.'-'.$reference_id; ?>">
								...
								if (strlen($jval) > 40) {
								?>
									<textarea rows="5" cols="170" style="min-width: 60%;" name="tn[<?php echo $ltag; ?>][<?php echo $reference_id; ?>][<?php echo $field; ?>][<?php echo $jkey; ?>]"><?php echo isset($tn_json_value[$jkey]) ? JHtml::_('esc_textarea', $tn_json_value[$jkey]) : ''; ?></textarea>
								<?php
								} else {
								?>
