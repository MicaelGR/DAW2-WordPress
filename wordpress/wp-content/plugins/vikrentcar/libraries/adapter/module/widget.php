...
jQuery(document).ready(function() {
	function registerWidgetScripts(form) {
		if (!form) {
			// if the form was not provided, find it using the widget ID
			form = jQuery('div[id$="{$this->id}"] form');
		}
	...
	jQuery('.widgets-chooser-add').on('click', function(e) {
		...
		var id = jQuery(parent).attr('id').match(/(.*?)__i__$/);
		...
		setTimeout(function() {
			...
			var createdForm = jQuery('div[id^="' + id.pop() + '"]').last();

			// find form within the box
			var _form = jQuery(createdForm).find('form');
	...
	if (!VIK_WIDGET_SAVE_LOOKUP.hasOwnProperty('{$this->_id}')) {
		...
		jQuery(document).ajaxSuccess(function(event, xhr, settings) {
			...
			var updatedForm = jQuery('div[id$="' + widget_id + '"]').find('form');
